...
class bvh_half : public hitable_half {
public:
    __device__ bvh_half() {}
    __device__ bvh_half(hitable_half* object) {}
    ...
    __device__ bvh_half(hitable_half** l, int n) {
        if (n == 1) {
            isLeaf = true;
            object = l[0];
            _bbox = AABB_half(l[0]->bbox());
            return;
        }

        AABB_half aabb = AABB_half(l[0]->bbox());
        for (int i = 1; i < n; i++)
        ...
        // n = 3 n/2=1
        int m = n / 2;
        _left = new bvh_half(l, m); // Not sure if we can give this to different cores/threads to parallelize
        _right = new bvh_half(l + m, n - m);

        isLeaf = false;
        _bbox = AABB_half(_left->bbox(), _right->bbox());
    }

    __device__ bool hit(const ray_half& r, half tmin, half tmax, hit_record_half& rec) const {
        if (isLeaf) {
            return object->hit(r, tmin, tmax, rec);
        }
        const bvh_half* current = this;
    ...
    bvh_half* _right;
protected:
    __device__ void sort_by_axis(hitable_half** list, int n, int axis) {
        for (int i = 1; i < n; i++) {
            hitable_half* obj = list[i];
            half obj_center;
    ...
    __device__ bool hit_node(const ray_half& r, half t_min, half t_max, hit_record_half& rec) const {
        if (!bbox().hit(r))
            return false;
        ...
        hit_record_half left_rec, right_rec;
